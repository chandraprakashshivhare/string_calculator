  ...
  describe 'Add string' do
    ...
    context 'when given a negative number' do
      it "throws an exception 'negatives not allowed' and includes the negative number" do
        expect { Calculator.new.add('1,-2') }.to raise_error(ArgumentError, 'negatives not allowed: -2')
      end
    end
    ...
    context 'when given a delimiter of any length' do
      it 'returns the sum of the numbers using the delimiter' do
        expect(Calculator.new.add("//[***]\n1***2***3")).to eq(6)
      end
    end

    context 'when given multiple delimiters' do
      it 'returns the sum of the numbers using the delimiters' do
        expect(Calculator.new.add("//[*][%]\n1*2%3")).to eq(6)
      end
    end

    context 'when given multiple delimiters with length longer than one character' do
      it 'returns the sum of the numbers using the delimiters' do
        expect(Calculator.new.add("//[**][%%]\n1**2%%3")).to eq(6)
      end
    end
